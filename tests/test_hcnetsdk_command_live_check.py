...
def test_load_inventory_reads_shell_quoted_env_file(tmp_path: Path) -> None:
    ...
    inventory = [
        {
            "name": "EZVIZ LC1C - Renier's Study Gate",
            "mdns": "G15225550",
            "password": "KKNUGT",
            "enc_key": "MlH9FXc_z7bd",
            "battery": False,
            "in_use": True,
        },
        {
    ...
    assert [item.serial for item in items] == ["G15225550", "D78293648"]
    assert items[0].name == "EZVIZ LC1C - Renier's Study Gate"
    assert items[1].enc_key == "q:62W8*Se{t["
...
def test_dry_run_redacts_command_password(tmp_path: Path) -> None:
    tool = _load_tool()
