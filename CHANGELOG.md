...
- Added built-in `app-lan-live-view` HCNetSDK command-port native-plan support, exposed through `hcnetsdk_command_port_native_lan_live_view_plan()` and `--hcnetsdk-command-native-plan app-lan-live-view`, so app-observed LAN preview captures no longer require a separate generated-plan JSON file.
